...
function cart_add($product_id, $quantity)
{
    if (isset($_SESSION['cart'][$product_id])) {
        //if cart exists item(s), increase the number of items $number
        $_SESSION['cart'][$product_id]['number'] += $quantity;
    } else {
        //get data from DB and save into cart
...
//update cart from user to system and from system to user
        ...
        if (!empty($product_of_user)) {
            foreach ($product_of_user as $product) {
                if (isset($_SESSION['cart'][$product['product_id']]) && mysqli_num_rows(mysqli_query($linkconnectDB, "SELECT product_id FROM cart_user WHERE product_id=" . $product['product_id']  . "")) == 1) {
                    //if cart exist item, the total item will increase $number
...
//sync item between session and db when user add to cart
function update_cart_user_db()
{
    ...
    //If row > 0, it means that user had item on db
    if (mysqli_num_rows(mysqli_query($linkconnectDB, "SELECT * FROM cart_user WHERE user_id=" . $user_nav . "")) > 0) {
        foreach ($cart as $product_cart) {
            ...
            //Loop cart_user when customer is a user (has account)
            ...
            if ($status == 1) { //if the item was listed on db -> edit
                $cart_user = array(
                    'id' => $cart_user['id'],
            ...
            } elseif ($status == 0) { //if the item hasn't been listed on db -> add
                $cart_user = array(
                    'id' => 0,
                    'user_id' => $user_nav,
                    'product_id' => $product_cart['id'],
